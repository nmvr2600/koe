...
除了保持精简，Koe Zen 还针对中国用户的使用习惯做了一些改进：

- **菜单栏 Provider 切换** —— 无需编辑配置文件，直接在菜单栏切换 Doubao（字节）和 Qwen（阿里）两家国内 ASR 服务商
- **通义千问 ASR 支持** —— 完整支持阿里云 DashScope 实时语音识别（Qwen3-ASR-Realtime）
- **中文优化配置** —— 默认系统提示词针对中文输入优化，更懂中文标点、数字转换和口语化表达
- **国内服务商友好** —— 配置模板直接提供火山引擎（Doubao）和阿里云（Qwen）的对接参数
